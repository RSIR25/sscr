...
class BaseModel(nn.Module):
    ...
    def forward(self):
        ...
        if self.config.loss == 'ml':
            self.fake_B, self.sar_feat = self.netG(self.real_A, batch_positions=self.dates)
        elif self.config.model == "uncrtaints":
            self.fake_B, self.sar_feat = self.netG(self.real_A, batch_positions=self.dates)
        elif self.config.model == "bdc":
            self.fake_B, self.sar_feat = self.netG(self.real_A, batch_positions=self.dates)
        else:
    ...
    def get_loss_G(self):
        ...
        if hasattr(self.netG, 'vars_idx'):
            if self.config.loss == "ml":
                self.loss_G, self.netG.variance = losses.calc_loss(self.criterion, self.config, self.fake_B[:, :, :self.netG.mean_idx, ...], self.real_B, var=self.sar_feat)
            else:
                self.loss_G, self.netG.variance = losses.calc_loss(self.criterion, self.config, self.fake_B[:, :, :self.netG.mean_idx, ...], self.real_B, var=self.sar_feat)
