...
class Conv_Extra(nn.Module):
    def __init__(self, channel, norm_layer, act_layer):
        ...
        self.block = nn.Sequential(nn.Conv2d(channel, channel//2, 1),
                                   nn.BatchNorm2d(channel//2),
                                #    nn.GroupNorm(num_channels=channel//2, num_groups=4),
                                   act_layer(),
                                   nn.Conv2d(channel//2, channel//2, 3, stride=1, padding=1, dilation=1, bias=False),
                                   nn.BatchNorm2d(channel//2),
                                   act_layer(),
...
class Scharr(nn.Module):
    def __init__(self, channel, norm_layer, act_layer):
        ...
        # self.norm = nn.GroupNorm(num_channels=channel, num_groups=4)
        self.act = act_layer()
...
class MultiSARv2(nn.Module):
    def __init__(
        ...
        super(MultiSARv2, self).__init__()
        ...
        self.multipro = True
        self.sar_input_dim = 2

        if decoder_widths is not None:
            assert encoder_widths[-1] == decoder_widths[-1]
        else: decoder_widths = encoder_widths

        op_encoder_width = encoder_widths[0]
        sar_encoder_width = encoder_widths[0]//4

        # ENCODER
        self.in_conv = ConvBlock(
            nkernels=[input_dim] + [op_encoder_width],
            k=1, s=1, p=0,
            norm=encoder_norm,
        )

        if self.multipro:
            self.sar_in_conv = ConvBlock(
                nkernels=[self.sar_input_dim] + [sar_encoder_width], # nkernels = [15, 32]
                k=1, s=1, p=0,
                # norm="instance",
                norm=encoder_norm,
            )
            self.sar_in_conv2 = ConvBlock(
                nkernels=[sar_encoder_width] + [sar_encoder_width], # nkernels = [15, 32]
                k=1, s=1, p=0,
                # norm="instance",
                norm=encoder_norm,
            )
            self.sar_edge_conv = Scharr(sar_encoder_width, encoder_norm, nn.GELU)

        if self.block_type=='mbconv':
            self.in_block = nn.ModuleList([MBConv(op_encoder_width, op_encoder_width, downsample=False, expansion=2, norm=encoder_norm) for layer in encoder_widths])
            if self.multipro == True:
                self.sar_in_block = nn.ModuleList([MBConv(sar_encoder_width, sar_encoder_width, downsample=False, expansion=2, norm=encoder_norm) for layer in encoder_widths])
                self.mask_block = MBConv(sar_encoder_width, sar_encoder_width, downsample=False, expansion=2, norm=encoder_norm)
                self.mask_conv = nn.Conv2d(sar_encoder_width, 1, 1, 1)
                # self.mask_beta = nn.Parameter(torch.tensor(0.5))
                self.mask_beta = 0.
    ...
    def forward(self, input, batch_positions=None):
        # input shape torch.Size([8, 1, 15, 256, 256])
        # batch_size, time step, channel(2SAR+13optical), height, width
        pad_mask = (
        ...
        if self.multipro == True:
            # sar encode
            sar_input = input[:,:,:2,...]
            sar_out = self.sar_in_conv.smart_forward(sar_input)
            sar_feat = sar_out
            sar_out = sar_out.squeeze(dim=1) # squeeze time dimension, now shape is [B, C, H, W]
            sar_out = self.sar_edge_conv(sar_out)
            sar_out = sar_out.unsqueeze(dim=1) # unsqueeze time dimension, now shape is [B, T=1, C, H, W]
            sar_out = self.sar_in_conv2.smart_forward(sar_out)
            for layer in self.sar_in_block:
                sar_out = layer.smart_forward(sar_out)

            # mask generation
            sar_mask = self.mask_block.smart_forward(sar_feat)
            sar_mask = sar_mask.squeeze(dim=1)
            sar_mask = self.mask_conv(sar_mask)
            sar_mask = torch.sigmoid(sar_mask)
            sar_mask = sar_mask.unsqueeze(dim=1)
        ...
        # fusion
        if self.multipro == True:
            out = (1 - self.mask_beta) * out + self.mask_beta * (out * sar_mask)
