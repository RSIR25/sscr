...
def get_loss(config):
    if config.loss == "GNLL":
        ...
        criterion1 = MultimodalLoss(reduction='mean')
        criterion = lambda pred, targ, var: criterion1(pred, targ, var)
...
def calc_loss(criterion, config, out, y, var=None):
    ...
    if config.loss in ['GNLL']:
        ...
        loss, variance = criterion(out, y, var)
    elif config.loss in ['ml']:
        loss, variance = criterion(out, y, var), None
    elif config.loss in ['bdc']:
...
def multimodal_loss(
    ...
    pix_loss = F.l1_loss(input, target, reduction=reduction)
    mean_sar_feat = var.mean(dim=2, keepdim=True) 
    mean_target = target.mean(dim=2, keepdim=True)
    flat_mean_sar_feat = mean_sar_feat.view(input.size(0), -1)  # shape: (B, H*W)
    flat_mean_target = mean_target.view(input.size(0), -1)

    pearson_corr = pearson_correlation(flat_mean_sar_feat, flat_mean_target, eps=1e-8)
    sar_loss = 1 - pearson_corr.mean()

    loss = 0.9 * pix_loss + 0.001 * sar_loss
    # loss = pix_loss
    return loss
...
def bdc_loss(
    ...
    pix_loss = F.l1_loss(input, target, reduction=reduction)
    temperature = (1. / 3407.) * torch.ones(1, 1)
    pred_bdc = BDCcal(input, temperature)
    gt_bdc = BDCcal(target, temperature)

    pred_bdc = F.normalize(pred_bdc, p=2, dim=1)
    gt_bdc = F.normalize(gt_bdc, p=2, dim=1)
    loss_bdc = - torch.mul(pred_bdc, gt_bdc).sum()

    loss = 1.0 * pix_loss +  loss_bdc * 1.0
    return loss
...
class MultimodalLoss(_Loss):
    ...
    def __init__(self, *, reduction: str = 'mean') -> None:
        super(MultimodalLoss, self).__init__(None, None, reduction)
    
    def forward(self, input: Tensor, sar_encode: Tensor, target: Tensor) -> Tensor:
        return multimodal_loss(input, sar_encode, target, reduction=self.reduction)
...
class BDCLoss(_Loss):
    ...
    def __init__(self, *, reduction: str = 'mean') -> None:
        super(BDCLoss, self).__init__(None, None, reduction)
    
    def forward(self, input: Tensor, target: Tensor) -> Tensor:
        return bdc_loss(input, target, reduction=self.reduction)