...
log_file = os.path.join(config.res_dir, config.experiment_name, "validation_logs.txt")
...
def save_tensor_as_heatmap(tensor, save_path="heatmap.png"):
    if tensor.dim() == 3:
        tensor = tensor.mean(dim=0)  
    
    heatmap = (tensor - tensor.min()) / (tensor.max() - tensor.min() + 1e-8)
    heatmap = (heatmap * 255).cpu().numpy().astype(np.uint8)
            ...
            with torch.no_grad():
                ...
                for bdx in range(batch_size):
                    ...
                    if config.plot_every>0 and idx % config.plot_every == 0:
                        ...
                        feat_i = feat[bdx][0].cpu()
                        save_path = os.path.join(plot_dir, f'img-{idx}_feat.png')
                        save_tensor_as_heatmap(feat_i, save_path)
                        
                        plot_img(x[bdx][:, :2, ...], 's1', plot_dir, file_id=idx)
